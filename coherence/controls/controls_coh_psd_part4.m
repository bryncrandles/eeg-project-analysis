...
filepattern = fullfile(datafolder, 'sub-43*_szcontrol_eeg_clean.set');

% directory of data files
datafiles = dir(filepattern);

% cell array to store coherence and psd 
coh_psd_ctrl_4 = cell(length(datafiles), 2);

% input files names into cell array
name = {datafiles.name}';
coh_psd_ctrl_4(:, 1) = name(1:length(datafiles));

% use window function in coherence estimates
window = 1;

% loop to calculate coh, psd for level 430 controls
parfor i = 1:length(datafiles)
    ...
    powerspd = cell(3, 1);
       ...
       coh_psd_ctrl_4{i, 2} = file_coh_psd;
end

save(['coh_psd_ctrl_4_' date '.mat'], 'coh_psd_ctrl_4', '-v7.3')
