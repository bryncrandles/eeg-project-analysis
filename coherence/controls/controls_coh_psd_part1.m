...
filepattern = fullfile(datafolder, 'sub-40*_szcontrol_eeg_clean.set'); 

% directory of data files
datafiles = dir(filepattern);

% cell array to store coherence and psd 
coh_psd_ctrl_1 = cell(length(datafiles), 2);

% names
name = {datafiles.name}';
coh_psd_ctrl_1(:, 1) = name(1:length(datafiles));

% start timer
tic

% loop to calculate coh, psd for first 14 files for resting, music and faces conditions 
parfor i = 1:length(datafiles)
       ...
       coh_psd_ctrl_1{i, 2} = file_coh_psd;
end

toc 

save('coh_psd_ctrl_1.mat', 'coh_psd_ctrl_1', '-v7.3')
